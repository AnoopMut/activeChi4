...
void S4Qt(double factor,double PARAMETER){
...
//	int FrameNum=(int)(Time*200.00);
					...
					if(r2<parameter){
						for(int n=0;n<N_Q;n++){														//Loop over all q's
							if(Degen[n]==1){
								...
								cosq[n]=cosq[n]+((double)(cos((qx*L*sphere[i].x0) + (qy*L*sphere[i].y0) + (qz*L*sphere[i].z0))));
								sinq[n]=sinq[n]+((double)(sin((qx*L*sphere[i].x0) + (qy*L*sphere[i].y0) + (qz*L*sphere[i].z0))));
							}	
							else if(Degen[n]==3){	
								double cosine=0.0;
								double sine=0.0;
								...
								qz = TWO_PI*invL*(double)(Vecz[n]);
								Q[n] = qx*qx + qy*qy + qz*qz;
								cosine+=((double)(cos((qx*L*sphere[i].x0) + (qy*L*sphere[i].y0) + (qz*L*sphere[i].z0))));
								sine+=((double)(sin((qx*L*sphere[i].x0) + (qy*L*sphere[i].y0) + (qz*L*sphere[i].z0))));
	...
	if(node==0){
		...
		sprintf(S4QT_file_name,"%s/S4qt_a_%.2lf.dat",sub_dir,PARAMETER);
	    ...
	    for(int k=0;k<num_of_frames;k++){
			if(counters[k]!=0){
			    for(int n=0;n<N_Q;n++){
    			    ...
    			    CosSqSum[k][n]=(double)Degen[n]*(CosSqSum[k][n]-CosSum[k][n]*CosSum[k][n])/(double)(N);
    			    CosSum[k][n]=(double)Degen[n]*CosSum[k][n]/(double)(N);
			        SinSum[k][n]=SinSum[k][n]/(double)(counters[k]);
   				    SinSqSum[k][n]=SinSqSum[k][n]/(double)(counters[k]);
    			   	SinSqSum[k][n]=(double)Degen[n]*(SinSqSum[k][n]-SinSum[k][n]*SinSum[k][n])/(double)(N);
    		    	SinSum[k][n]=(double)Degen[n]*SinSum[k][n]/(double)(N);
...
int GetQ(){
	...
	do { //Loop over the partial sorts.
		inc /= 3;
		for (i=inc+1;i<=count;i++) { //Outer loop of straight insertion.
			v=q[i];
			vec[0]=Vecx[i];
			vec[1]=Vecy[i];
			vec[2]=Vecz[i];
			j=i;
			while (q[j-inc] > v) { //Inner loop of straight insertion.
			q[j]=q[j-inc];
			Vecx[j]=Vecx[j-inc];
			Vecy[j]=Vecy[j-inc];
			Vecz[j]=Vecz[j-inc];
